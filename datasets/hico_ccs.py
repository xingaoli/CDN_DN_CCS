...
class HICODetection(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        if np.random.random() < 0.25 and self.img_set == 'train' and self.ids[idx] not in self.nohoi_index:
            sim_im_num = 3
            random_index = [self.ids[idx]] + get_sim_index(sim_im_num, self.nohoi_index, self.sim_index[self.ids[idx]])

            img_anno, img = get_replace_image(random_index, self.annotations, self.img_folder, 'hico')
        ...
        for idx, img_anno in enumerate(self.annotations):
            for hoi in img_anno['hoi_annotation']:
                if hoi['subject_id'] >= len(img_anno['annotations']) or hoi['object_id'] >= len(
                        img_anno['annotations']) or hoi['subject_id'] >= 64 or hoi['object_id'] >= 64:
                    nohoi_index.append(idx)
                    break
        self.nohoi_index = nohoi_index

    def get_sim_index(self):
        self.sim_index = pickle.load(open('datasets/sim_index_hico.pickle', 'rb'))
...
def make_hico_transforms(image_set):
    ...
    if image_set == 'val':
        return T.Compose([
            T.RandomResize([int(800)], max_size=int(1333)),
            normalize,
        ])
