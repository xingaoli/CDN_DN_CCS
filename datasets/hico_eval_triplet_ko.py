...
class HICOEvaluator():
    def __init__(self, preds, gts, rare_triplets, non_rare_triplets, correct_mat, args):
        ...
        for index, img_preds in enumerate(preds):
            ...
            #bboxes = [{'bbox': list(bbox), 'category_id': label} for bbox, label inzip(img_preds['boxes'], img_preds['labels'])]
            ...
            '''ol = img_preds['obj_labels'][:, self.hoi_obj_list].ravel()
            sl = np.full_like(ol, self.subject_category_id)
            category_id_bb = np.concatenate(sl, ol)'''
    ...
    def evaluate_ko(self):
        ...
        for img_preds, img_gts in zip(self.preds, self.gts):
            ...
            if len(gt_bboxes) != 0:
                bbox_pairs, bbox_overlaps = self.compute_iou_mat(gt_bboxes, pred_bboxes)
                self.compute_fptp_ko(pred_hois, gt_hois, bbox_pairs, bbox_overlaps, cat_ids, pred_bboxes)
            else:
                for pred_hoi in pred_hois:
                    triplet = pred_hoi['category_id']
                    if triplet not in self.gt_triplets:
                        continue
                    triplet_obj_id = self.hoi_obj_list[triplet]
                    if triplet_obj_id not in cat_ids:
                        continue
                    self.tp[triplet].append(0)
                    self.fp[triplet].append(1)
                    self.score[triplet].append(pred_hoi['score'])
    ...
    def compute_map_ko(self):
        ...
        print('--------------------')
        if self.zero_shot_type == "default":
            print('KO mAP full: {} mAP rare: {}  mAP non-rare: {}  mean max recall: {}'.format(m_ap, m_ap_rare, m_ap_non_rare,
                                                                                    m_max_recall))
            return_dict = {'mAP': m_ap, 'mAP rare': m_ap_rare, 'mAP non-rare': m_ap_non_rare, 'mean max recall': m_max_recall}
    ...
    def compute_fptp_ko(self, pred_hois, gt_hois, match_pairs, bbox_overlaps, cat_ids,  pred_bboxes):
        pos_pred_ids = match_pairs.keys()
        vis_tag = np.zeros(len(gt_hois))
        pred_hois.sort(key=lambda k: (k.get('score', 0)), reverse=True)
        if len(pred_hois) != 0:
