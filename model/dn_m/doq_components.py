...
def encoder_gt_postion(targets, batch_size):
    ...
    for i in range(batch_size):
        ...
        sub_boxes = sub_boxes.repeat(scale, 1)
        obj_boxes = obj_boxes.repeat(scale, 1)

        sub_diff = torch.zeros_like(sub_boxes)
        sub_diff[:, :2] = sub_boxes[:, 2:] / 2
        sub_diff[:, 2:] = sub_boxes[:, 2:]
        sub_boxes += torch.mul((torch.rand_like(sub_boxes) * 2 - 1.0), sub_diff) * 0.4
        ...
        obj_boxes += torch.mul((torch.rand_like(obj_boxes) * 2 - 1.0), obj_diff) * 0.4
...
def prepare_for_doq(doq_args, batch_size, bbox_enc, query_embedding_weight):
    targets = copy.deepcopy(doq_args[0])
    ...
    scale = 5
    single_pad = gt_positions_padding_size // scale

    for i in range(batch_size):
        gt_position = gt_positions[i]
        gt_single_size = len(gt_position) // scale
        if len(gt_position) > 0:
            for j in range(scale):
                gt_positions_arrays[j*single_pad:j*single_pad+gt_single_size, i, :] = gt_position[gt_single_size*j:gt_single_size*(j+1)]
    ...
    for i in range(scale):
        attn_mask[i*single_pad:(i+1)*single_pad, i*single_pad:(i+1)*single_pad] = 0
...
def _neg_loss(pred, gt, alpha=0.25):
...
def compute_doq_loss(mask_dict, aux_num):
    ...
    if 'output_known_gt' in mask_dict:
        ...
        for i, t in enumerate(targets):
            t['sub_boxes'] = t['sub_boxes'].repeat(scale, 1)
            t['obj_boxes'] = t['obj_boxes'].repeat(scale, 1)
            t['obj_labels'] = t['obj_labels'].repeat(scale)
            t['verb_labels'] = t['verb_labels'].repeat(scale, 1)
        ...
        single_pad = outputs['gt_obj_logits'].shape[1] // scale

        for i, t in enumerate(targets):
            gt_single_size = len(t['sub_boxes']) // scale
            gt_batch_idx.extend([i] * len(t['sub_boxes']))
            single_indices = torch.arange(0, gt_single_size)
            for j in range(scale):
                gt_indices.extend(single_indices+(j*single_pad))
