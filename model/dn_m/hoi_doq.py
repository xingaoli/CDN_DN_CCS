...
from .cdn_doq import build_cdn
from .doq_components import prepare_for_doq, doq_post_process, compute_doq_loss
...
class CDNHOI(nn.Module):
    ...
    def forward(self, samples: NestedTensor, doq_args=None):
        ...
        if doq_args is not None:
            ...
            input_query_with_gt_position_query, attn_mask, tgt_padding_mask, mask_dict = \
                prepare_for_doq(doq_args, src.size(0), self.bbox_enc, self.query_embed.weight)
            ####
            hopd_out, interaction_decoder_out = self.transformer(self.input_proj(src), mask,
                                        input_query_with_gt_position_query, pos[-1], None, attn_mask)[:2]
...
class SetCriterionHOI(nn.Module):
    ...
    def forward(self, outputs, targets, mask_dict=None):
        ...
        # todo doq loss compute
        if mask_dict is not None:
            aux_num = 0
            if 'aux_outputs' in outputs:
                aux_num = len(outputs['aux_outputs'])

            doq_losses = compute_doq_loss(mask_dict, aux_num)
            losses.update(doq_losses)
